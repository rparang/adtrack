  ...
  this.getReports(function(report_names) {
    report_names.forEach(function(report_name, i) {
      utils.readFile(report_name, function(report) {
        ...
        for (var property in report.report) {
          if (!(report.report.hasOwnProperty(property))) {
            console.log(property)
            tracker_company[property] = 0;
          }
          else {
            tracker_company[property] += 1;
            console.log(tracker_company)
          }
        }
      })
    })
...
adtrack.getTsvOfTrackers(function() { });
